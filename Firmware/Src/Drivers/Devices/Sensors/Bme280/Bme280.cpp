...
Driver::Bme280ErrorCodes Driver::Bme280::_readCompensationData(void)
{
    ...
    if (m_interface.recvData(BME280_COMPENS_P1, &m_buffer[0], 
    ...
    if (m_interface.recvByte(BME280_COMPENS_H1, &m_buffer[0])) {
        return BME280_ERR_COMPENS_H1;
    }
    m_humidityCompensData[0] = m_buffer[0];
    memset(&m_buffer[0], 0x00, 7);
    if (m_interface.recvData(BME280_COMPENS_H2, &m_buffer[0], 7)) {
...
void Driver::Bme280::_getTemperature(bool isKelvin)
{
    ...
    if (m_interface.recvData(BME280_TEMPERATURE_MSB, &m_buffer[0], 3)) {
        return;
    }
...
void Driver::Bme280::_getPressure(bool isMmhg)
{
    ...
    if (m_interface.recvData(BME280_PRESSURE_MSB, &m_buffer[0], 3)) {
        return;
    }
...
void Driver::Bme280::_getHumidity(void)
{
    ...
    if (m_interface.recvData(BME280_HUMIDITY_MSB, &m_buffer[0], 2)) {
        return;
    }
